...
const onLoad = (rootId: string, words: string[], wordDelay: number, letterDelay: number) => {
    ...
    function typeLetter(element: HTMLElement, words: string[], letter: number, word: number) {
        setTimeout(() => {
            ...
            if (letter < currentWord.length - 1) typeLetter(element, words, letter + 1, word);
            else {
                setTimeout(() => {
    ...
    function eraseLetter(element: HTMLElement, words: string[], letter: number, word: number) {
        setTimeout(() => {
            ...
            if (letter > 0) eraseLetter(element, words, letter - 1, word);
            else if (word < words.length - 1) typeLetter(element, words, 0, word + 1);
            else typeLetter(element, words, 0, 0);
        }, letterDelay);
    }
};
        ...
        <div class={`overflow-hidden ${!useBackground && 'bg-primary'} leading-[120%] text-primary-content`}>
            <div class="relative flex justify-center items-center w-full h-[86vw] lg:h-[43vw]">
                ...
                {useBackground == "image" && backgroundImage?.src && <Image
                    ...
                    height={backgroundImage.height || 618}
                    src={backgroundImage.src}
                    alt={backgroundImage.alt || "background image"}
                    class="object-cover object-top w-full absolute top-0 left-0 -z-10"
                />}
                <script
                    type="module"
                    dangerouslySetInnerHTML={{ __html: useScript(onLoad, id, titleTyping, wordDelay || 3000, letterDelay || 100) }}
                />
                <AnimateOnShow
                        ...
                        >
                            <div class="mb-6 hidden lg:flex gap-7">
                                {slidesTitleIcon && slidesTitleIcon.src && <Image
                                    ...
                                    src={slidesTitleIcon.src}
                                    alt={slidesTitleIcon.alt || "slides title icon"}
